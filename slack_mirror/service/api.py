...
def _tenant_settings_html(*, auth_session: FrontendAuthSession, tenants: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "function tenantCardHtml(item){const name=String(item.name||'unknown');const domain=String(item.domain||'');const enabled=!!item.enabled;const credentialReady=!!item.credential_ready;const synced=!!item.db_synced;const missing=(item.missing_required_credentials||[]).join(', ')||'-';const manifest=item.manifest||{};const nextAction=String(item.next_action||'unknown');const health=item.health||{};const syncHealth=item.sync_health||{};const backfillStatus=item.backfill_status||{};const dbStats=item.db_stats||{};const liveUnits=item.live_units||{};const validationStatus=String(item.validation_status||'unknown');const pendingAction=pendingTenantActions.get(name)||'';const webhooksState=String(liveUnits.webhooks||'unknown');const daemonState=String(liveUnits.daemon||'unknown');const healthTone=String(health.tone||'neutral');const syncTone=String(syncHealth.tone||'neutral');const backfillTone=String(backfillStatus.tone||'neutral');const canActivate=!pendingAction&&nextAction==='ready_to_activate';const unitsActive=webhooksState==='active'||daemonState==='active';const canStartLive=!pendingAction&&enabled&&!unitsActive;const canRestartLive=!pendingAction&&enabled&&unitsActive;const canStopLive=!pendingAction&&enabled&&unitsActive;const canBackfill=!pendingAction&&enabled&&synced;const manifestButton=`<button data-tenant-copy-manifest=\"${name}\">Copy Manifest JSON</button><div class='hint'>Slack only accepts pasted manifest JSON. This copies the rendered manifest to your clipboard.</div>`;const dbSummary=`${Number(dbStats.channels||0)} channels · ${Number(dbStats.messages||0)} messages · ${Number(dbStats.files||0)} files`;const dbDetail=`attachment text ${Number(dbStats.attachment_text||0)} · OCR ${Number(dbStats.ocr_text||0)} · embedding pending ${Number(dbStats.embedding_pending||0)} · derived pending ${Number(dbStats.derived_pending||0)}`;let actionRows='';if(canActivate){actionRows+=`<div class='button-row'><button data-tenant-activate=\"${name}\">Activate + start live units</button></div><div class='hint'>Credentials and config are ready. Activation enables the tenant and installs the live units.</div>`;}else if(pendingAction==='activate'){actionRows+=`<div class='hint'>Activation is in progress. This tile will refresh as the live units settle.</div>`;}else if(!enabled){actionRows+=`<div class='hint'>Activation appears when credentials are ready and the tenant is synced into the DB.</div>`;}const liveButtons=[];if(canStartLive)liveButtons.push(`<button data-tenant-live=\"${name}\" data-live-action='start'>Start live sync</button>`);if(canRestartLive)liveButtons.push(`<button data-tenant-live=\"${name}\" data-live-action='restart'>Restart live sync</button>`);if(canStopLive)liveButtons.push(`<button data-tenant-live=\"${name}\" data-live-action='stop'>Stop live sync</button>`);if(liveButtons.length)actionRows+=`<div class='button-row'>${liveButtons.join('')}</div>`;else if(pendingAction==='start'||pendingAction==='restart'||pendingAction==='stop'){actionRows+=`<div class='hint'>Live sync action <code>${pendingAction}</code> is in progress. This tile is polling for the updated unit state.</div>`;}const workButtons=[];if(canBackfill)workButtons.push(`<button data-tenant-backfill=\"${name}\">Run bounded backfill</button>`);workButtons.push(`<button class='danger' title='Retire tenant' data-tenant-retire=\"${name}\">&#128465; Retire tenant</button>`);actionRows+=`<div class='button-row'>${workButtons.join('')}</div>`;if(pendingAction==='backfill')actionRows+=`<div class='hint'>Bounded backfill has started. This tile is polling for queue and DB-stat changes.</div>`;return `<article class='tenant-card tenant-card-full' data-tenant-card=\"${name}\"><div class='tenant-head'><h2>${name}</h2><span class='badge ${badgeClass(enabled?'ok':'warn')}'>${enabled?'enabled':'disabled'}</span></div><div class='meta'>Slack domain <code>${domain}</code></div><div class='tenant-grid'><div class='status-block'><strong>Credentials</strong><div class='meta'>${credentialReady?'ready':'missing'} · missing <code>${missing}</code></div><div class='status-strip'><span class='${pillClass(credentialReady?'ok':'warn')}'>${credentialReady?'ready':'needs credentials'}</span></div></div><div class='status-block'><strong>Mirrored DB</strong><div class='meta'>${dbSummary}</div><div class='hint'>${dbDetail}</div><div class='status-strip'><span class='status-pill neutral'>DB ${String(synced).toLowerCase()}</span><span class='status-pill neutral'>embedding errors ${Number(dbStats.embedding_errors||0)}</span><span class='status-pill neutral'>derived errors ${Number(dbStats.derived_errors||0)}</span></div></div><div class='status-block'><strong>Backfill</strong><div class='meta'>${String(backfillStatus.summary||'No backfill status available.')}</div><div class='hint'>${String(backfillStatus.detail||'')}</div><div class='status-strip'><span class='${pillClass(backfillTone)}'>${String(backfillStatus.label||'unknown')}</span></div></div><div class='status-block'><strong>Sync status</strong><div class='meta'>${String(syncHealth.summary||'No sync status available.')}</div><div class='hint'>${String(syncHealth.detail||'')}</div><div class='status-strip'><span class='${pillClass(syncTone)}'>${String(syncHealth.label||'unknown')}</span><span class='status-pill neutral'>webhooks ${webhooksState}</span><span class='status-pill neutral'>daemon ${daemonState}</span></div></div><div class='status-block'><strong>Health</strong><div class='meta'>${String(health.summary||'No health status available.')}</div><div class='hint'>${String(health.detail||'')}</div><div class='status-strip'><span class='${pillClass(healthTone)}'>${validationStatus}</span></div></div><div class='status-block'><strong>Next action</strong><div class='meta'><code>${nextAction}</code></div></div><div class='status-block'><strong>Manifest</strong><div class='meta'>${manifestButton}<div class='hint'><code>${String(manifest.path||'')}</code></div></div></div></div>${actionRows}</article>`;}"
        "function bindTenantActions(){for(const manifestButton of document.querySelectorAll('button[data-tenant-copy-manifest]')){manifestButton.onclick=async()=>{const name=manifestButton.getAttribute('data-tenant-copy-manifest');const label=manifestButton.textContent;manifestButton.disabled=true;manifestButton.textContent='copying...';try{const resp=await fetch(`/v1/tenants/${encodeURIComponent(name)}/manifest`);const data=await resp.json().catch(()=>({error:{message:'Manifest copy failed'}}));if(!resp.ok){showTenantFeedback(data.error?.message||'Manifest copy failed',true);return;}await navigator.clipboard.writeText(String(data.content||''));showTenantFeedback(`Copied manifest JSON for ${name}.`,false);}catch(error){showTenantFeedback('Clipboard write failed on this browser.',true);}finally{manifestButton.disabled=false;manifestButton.textContent=label;}};}for(const activateButton of document.querySelectorAll('button[data-tenant-activate]')){activateButton.onclick=async()=>{const name=activateButton.getAttribute('data-tenant-activate');setTenantPendingAction(name,'activate');renderTenantList(window.latestTenants||initialTenants);try{const resp=await fetch(`/v1/tenants/${encodeURIComponent(name)}/activate`,{method:'POST',headers:{'content-type':'application/json'},body:'{}'});const data=await resp.json().catch(()=>({error:{message:'Tenant activation failed'}}));if(!resp.ok){clearTenantPendingAction(name);await refreshTenants();showTenantFeedback(data.error?.message||'Tenant activation failed',true);return;}showTenantFeedback(`Activated ${data.tenant.name}. Polling for live-unit status...`,false);await refreshTenants();startTenantPolling(8,1500);}catch(error){clearTenantPendingAction(name);await refreshTenants();showTenantFeedback('Tenant activation failed',true);}};}for(const liveButton of document.querySelectorAll('button[data-tenant-live]')){liveButton.onclick=async()=>{const name=liveButton.getAttribute('data-tenant-live');const action=liveButton.getAttribute('data-live-action');setTenantPendingAction(name,action);await refreshTenants();try{const resp=await fetch(`/v1/tenants/${encodeURIComponent(name)}/live`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action})});const data=await resp.json().catch(()=>({error:{message:'Live action failed'}}));if(!resp.ok){clearTenantPendingAction(name);await refreshTenants();showTenantFeedback(data.error?.message||'Live action failed',true);return;}showTenantFeedback(`Live action ${data.action} completed for ${data.tenant.name}. Polling for updated status...`,false);await refreshTenants();startTenantPolling(6,1500);}catch(error){clearTenantPendingAction(name);await refreshTenants();showTenantFeedback('Live action failed',true);}};}for(const backfillButton of document.querySelectorAll('button[data-tenant-backfill]')){backfillButton.onclick=async()=>{const name=backfillButton.getAttribute('data-tenant-backfill');setTenantPendingAction(name,'backfill');await refreshTenants();try{const resp=await fetch(`/v1/tenants/${encodeURIComponent(name)}/backfill`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({auth_mode:'user',include_messages:true,include_files:false,channel_limit:10})});const data=await resp.json().catch(()=>({error:{message:'Backfill failed'}}));if(!resp.ok){clearTenantPendingAction(name);await refreshTenants();showTenantFeedback(data.error?.message||'Backfill failed',true);return;}showTenantFeedback(`Bounded backfill started for ${data.tenant.name}. Polling DB and queue stats...`,false);await refreshTenants();startTenantPolling(10,2000);}catch(error){clearTenantPendingAction(name);await refreshTenants();showTenantFeedback('Backfill failed',true);}};}for(const retireButton of document.querySelectorAll('button[data-tenant-retire]')){retireButton.onclick=async()=>{const name=retireButton.getAttribute('data-tenant-retire');const decision=await requestBrowserDialog({title:`Retire tenant ${name}`,message:'This removes the tenant from config. Type the tenant name to confirm. Optionally also delete mirrored DB rows for the retired tenant.',inputLabel:'Confirm tenant name',inputPlaceholder:name,requiredText:name,requiredTextError:`Type ${name} to retire this tenant.`,checkboxLabel:'Also delete mirrored DB rows for this tenant',confirmLabel:'Retire tenant',cancelLabel:'Cancel',danger:true});if(!decision.confirmed)return;retireButton.disabled=true;retireButton.textContent='retiring...';try{const resp=await fetch(`/v1/tenants/${encodeURIComponent(name)}/retire`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({confirm:name,delete_db:decision.checked})});const data=await resp.json().catch(()=>({error:{message:'Retire failed'}}));if(!resp.ok){showTenantFeedback(data.error?.message||'Retire failed',true);return;}showTenantFeedback(`Retired ${data.tenant.name}. DB deleted: ${data.db_deleted?'yes':'no'}.`,false);await refreshTenants();}finally{retireButton.disabled=false;retireButton.textContent='Retire tenant';}};}}"
