...
def _runtime_reports_index_html(
    ...
    *,
    ...
    return (
        ...
        "function setInlineManagerBusyState(current,config,isBusy){for(const attr of [config.renameToggleAttr,config.renameCancelAttr,config.renameSaveAttr,config.deleteAttr]){for(const el of document.querySelectorAll(`[${attr}=\"${CSS.escape(current)}\"]`)){if('disabled' in el)el.disabled=isBusy;el.dataset.busy=isBusy?'true':'false';}}const input=document.getElementById(`${config.renameInputPrefix}${current}`);if(input)input.disabled=isBusy;}"
        "function bindInlineManagerActions(scope,config){for(const button of scope.querySelectorAll(`[${config.renameToggleAttr}]`)){if(button.dataset.bound==='true')continue;button.dataset.bound='true';button.addEventListener('click',()=>{if(button.dataset.busy==='true')return;const current=button.getAttribute(config.renameToggleAttr);if(!current)return;const row=document.getElementById(`${config.renameRowPrefix}${current}`);if(row)row.hidden=false;const input=document.getElementById(`${config.renameInputPrefix}${current}`);if(input){input.focus();input.select();}});}for(const button of scope.querySelectorAll(`[${config.renameCancelAttr}]`)){if(button.dataset.bound==='true')continue;button.dataset.bound='true';button.addEventListener('click',()=>{if(button.dataset.busy==='true')return;const current=button.getAttribute(config.renameCancelAttr);if(!current)return;const row=document.getElementById(`${config.renameRowPrefix}${current}`);if(row)row.hidden=true;const input=document.getElementById(`${config.renameInputPrefix}${current}`);if(input)input.value=current;});}for(const button of scope.querySelectorAll(`[${config.renameSaveAttr}]`)){if(button.dataset.bound==='true')continue;button.dataset.bound='true';button.addEventListener('click',async()=>{if(button.dataset.busy==='true')return;const current=button.getAttribute(config.renameSaveAttr);if(!current)return;const input=document.getElementById(`${config.renameInputPrefix}${current}`);const next=(input?.value||'').trim();if(!next||next===current){const row=document.getElementById(`${config.renameRowPrefix}${current}`);if(row)row.hidden=true;return;}setInlineManagerBusyState(current,config,true);try{const resp=await fetch(config.renamePath(current,next),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(config.renameBody(next))});if(resp.ok){config.applyRename(current,next);config.setFeedback(`Renamed ${config.itemLabel} ${current} to ${next}.`,false);return;}const data=await resp.json().catch(()=>({error:{message:'Rename failed'}}));config.setFeedback(data.error?.message||'Rename failed',true);}finally{setInlineManagerBusyState(current,config,false);}});}for(const button of scope.querySelectorAll(`[${config.deleteAttr}]`)){if(button.dataset.bound==='true')continue;button.dataset.bound='true';button.addEventListener('click',async()=>{if(button.dataset.busy==='true')return;const current=button.getAttribute(config.deleteAttr);if(!current)return;if(!window.confirm(config.deleteConfirm(current)))return;setInlineManagerBusyState(current,config,true);try{const resp=await fetch(config.deletePath(current),{method:'DELETE'});if(resp.ok){config.removeRow(current);config.setFeedback(`Deleted ${config.itemLabel} ${current}.`,false);return;}const data=await resp.json().catch(()=>({error:{message:'Delete failed'}}));config.setFeedback(data.error?.message||'Delete failed',true);}finally{setInlineManagerBusyState(current,config,false);}});}}"
...
def _exports_index_html(exports: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "function setCreateExportBusyState(isBusy){if(createExportButton)createExportButton.disabled=isBusy;workspaceSelect.disabled=isBusy;channelFilterInput.disabled=isBusy||!workspaceChannels.length;channelSelect.disabled=isBusy||channelSelect.options.length<=1;dayInput.disabled=isBusy;const tzInput=document.getElementById('export-tz');if(tzInput)tzInput.disabled=isBusy;const audienceSelect=document.getElementById('export-audience');if(audienceSelect)audienceSelect.disabled=isBusy;const exportIdInput=document.getElementById('export-id');if(exportIdInput)exportIdInput.disabled=isBusy;}"
