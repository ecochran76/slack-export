...
def _runtime_reports_index_html(
    ...
    *,
    ...
    return (
        ...
        "<div class='hint bad' id='report-create-error' hidden></div>"
        "</section>"
        "<section>"
        f"{header_links}"
        f"{table}"
        ...
        "function validateReportCreateForm(name,baseUrl,timeout){if(!name)return 'Report name is required';if(!baseUrl)return 'Base URL is required';if(!Number.isFinite(timeout)||timeout<=0)return 'Timeout seconds must be greater than zero';return '';}"
        ...
        "reportNameInput.addEventListener('input',()=>setReportCreateError(''));"
        "if(baseUrlSelect)baseUrlSelect.addEventListener('change',()=>setReportCreateError(''));"
        "const reportTimeoutInput=document.getElementById('report-timeout');"
        "if(reportTimeoutInput)reportTimeoutInput.addEventListener('input',()=>setReportCreateError(''));"
        ...
        "if(validationError){setReportCreateError(validationError);setReportFeedback(validationError,true);return;}"
        "setCreateReportBusyState(true);"
        "try{const resp=await fetch('/v1/runtime/reports',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name,base_url:baseUrl,timeout})});if(resp.ok){const data=await resp.json().catch(()=>({report:null}));setReportCreateError('');if(data.report){insertCreatedReport(data.report);reportNameInput.value=timestampedReportName();setReportFeedback(`Created runtime report ${data.report.name}.`,false);return;}setReportFeedback('Created runtime report.',false);return;}const data=await resp.json().catch(()=>({error:{message:'Create failed'}}));const message=data.error?.message||'Create failed';setReportCreateError(message);setReportFeedback(message,true);}finally{setCreateReportBusyState(false);}"
        "});"
        "function bindReportRowActions(scope){bindInlineManagerActions(scope,{renameToggleAttr:'data-report-rename-toggle',renameCancelAttr:'data-report-rename-cancel',renameSaveAttr:'data-report-rename-save',deleteAttr:'data-report-delete',renameInputPrefix:'rename-input-',renameRowPrefix:'rename-row-',rowErrorPrefix:'report-row-error-',renamePath:(current)=>`/v1/runtime/reports/${encodeURIComponent(current)}/rename`,renameBody:(next)=>({name:next}),deletePath:(current)=>`/v1/runtime/reports/${encodeURIComponent(current)}`,deleteConfirm:(current)=>`Delete runtime report ${current}?`,applyRename:applyReportRename,removeRow:removeReportRow,setFeedback:setReportFeedback,itemLabel:'runtime report'});}"
        "function applyReportRename(current,next){const row=document.getElementById(`report-row-${current}`);if(!row)return;row.id=`report-row-${next}`;row.dataset.reportName=next;const nameCell=row.querySelector('[data-report-col=\"name\"]');if(nameCell){nameCell.innerHTML=`<a href=\"/runtime/reports/${encodeURIComponent(next)}\">${escapeHtml(next)}</a>`;}const linksCell=row.querySelector('[data-report-col=\"links\"]');if(linksCell){linksCell.innerHTML=`<a href=\"/runtime/reports/${encodeURIComponent(next)}.latest.md\">md</a> <a href=\"/runtime/reports/${encodeURIComponent(next)}.latest.json\">json</a>`;}for(const el of row.querySelectorAll('[data-report-rename-toggle]'))el.setAttribute('data-report-rename-toggle',next);for(const el of row.querySelectorAll('[data-report-rename-save]'))el.setAttribute('data-report-rename-save',next);for(const el of row.querySelectorAll('[data-report-rename-cancel]'))el.setAttribute('data-report-rename-cancel',next);for(const el of row.querySelectorAll('[data-report-delete]'))el.setAttribute('data-report-delete',next);const renameRow=document.getElementById(`rename-row-${current}`);if(renameRow){renameRow.id=`rename-row-${next}`;renameRow.hidden=true;}const renameInput=document.getElementById(`rename-input-${current}`);if(renameInput){renameInput.id=`rename-input-${next}`;renameInput.value=next;renameInput.setAttribute('aria-label',`Rename ${next}`);}const rowError=document.getElementById(`report-row-error-${current}`);if(rowError){rowError.id=`report-row-error-${next}`;rowError.hidden=true;rowError.textContent='';}}"
...
def _exports_index_html(exports: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "<div class='hint bad' id='export-create-error' hidden></div>"
        "</section>"
        ...
        "function validateExportCreatePayload(payload){if(!payload.workspace)return 'Workspace is required';if(!payload.channel)return 'Channel is required';if(!payload.day)return 'Day is required';if(!/^\\d{4}-\\d{2}-\\d{2}$/.test(payload.day))return 'Day must use YYYY-MM-DD';if(!payload.tz)return 'Timezone is required';if(!payload.audience)return 'Audience is required';return '';}"
        ...
        "workspaceSelect.addEventListener('change',()=>setExportCreateError(''));"
        "channelFilterInput.addEventListener('input',()=>setExportCreateError(''));"
        "channelSelect.addEventListener('change',()=>setExportCreateError(''));"
        "dayInput.addEventListener('input',()=>setExportCreateError(''));"
        "const exportTzInput=document.getElementById('export-tz');"
        "if(exportTzInput)exportTzInput.addEventListener('input',()=>setExportCreateError(''));"
        "const exportAudienceSelect=document.getElementById('export-audience');"
        "if(exportAudienceSelect)exportAudienceSelect.addEventListener('change',()=>setExportCreateError(''));"
        ...
        "if(validationError){setExportCreateError(validationError);setExportFeedback(validationError,true);return;}"
        "setCreateExportBusyState(true);"
        "try{const resp=await fetch('/v1/exports',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});if(resp.ok){const data=await resp.json().catch(()=>({export:null}));setExportCreateError('');if(data.export){insertCreatedExport(data.export);document.getElementById('export-id').value='';setExportFeedback(`Created export ${data.export.export_id}.`,false);return;}setExportFeedback('Created export.',false);return;}const data=await resp.json().catch(()=>({error:{message:'Create failed'}}));const message=data.error?.message||'Create failed';setExportCreateError(message);setExportFeedback(message,true);}finally{setCreateExportBusyState(false);}"
        "});"
        "function bindExportRowActions(scope){bindInlineManagerActions(scope,{renameToggleAttr:'data-export-rename-toggle',renameCancelAttr:'data-export-rename-cancel',renameSaveAttr:'data-export-rename-save',deleteAttr:'data-export-delete',renameInputPrefix:'export-rename-input-',renameRowPrefix:'export-rename-row-',rowErrorPrefix:'export-row-error-',renamePath:(current)=>`/v1/exports/${encodeURIComponent(current)}/rename`,renameBody:(next)=>({export_id:next,audience:'local'}),deletePath:(current)=>`/v1/exports/${encodeURIComponent(current)}`,deleteConfirm:(current)=>`Delete export ${current}?`,applyRename:applyExportRename,removeRow:removeExportRow,setFeedback:setExportFeedback,itemLabel:'export'});}"
        "function applyExportRename(current,next){const row=document.getElementById(`export-row-${current}`);if(!row)return;row.id=`export-row-${next}`;row.dataset.exportId=next;const nameCell=row.querySelector('[data-export-col=\"name\"]');if(nameCell){nameCell.innerHTML=`<a href=\"/exports/${encodeURIComponent(next)}\">${escapeHtml(next)}</a>`;}const manifestCell=row.querySelector('[data-export-col=\"manifest\"]');if(manifestCell){manifestCell.innerHTML=`<a href=\"/v1/exports/${encodeURIComponent(next)}\">manifest</a>`;}for(const el of row.querySelectorAll('[data-export-rename-toggle]'))el.setAttribute('data-export-rename-toggle',next);for(const el of row.querySelectorAll('[data-export-rename-save]'))el.setAttribute('data-export-rename-save',next);for(const el of row.querySelectorAll('[data-export-rename-cancel]'))el.setAttribute('data-export-rename-cancel',next);for(const el of row.querySelectorAll('[data-export-delete]'))el.setAttribute('data-export-delete',next);const renameRow=document.getElementById(`export-rename-row-${current}`);if(renameRow){renameRow.id=`export-rename-row-${next}`;renameRow.hidden=true;}const renameInput=document.getElementById(`export-rename-input-${current}`);if(renameInput){renameInput.id=`export-rename-input-${next}`;renameInput.value=next;renameInput.setAttribute('aria-label',`Rename ${next}`);}const rowError=document.getElementById(`export-row-error-${current}`);if(rowError){rowError.id=`export-row-error-${next}`;rowError.hidden=true;rowError.textContent='';}}"
