...
def _runtime_reports_index_html(
    ...
    *,
    ...
    return (
        ...
        "try{const resp=await fetch('/v1/runtime/reports',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name,base_url:baseUrl,timeout})});if(resp.ok){const data=await resp.json().catch(()=>({report:null}));if(data.report){insertCreatedReport(data.report);reportNameInput.value=timestampedReportName();setReportFeedback(`Created runtime report ${data.report.name}.`,false);return;}setReportFeedback('Created runtime report.',false);return;}const data=await resp.json().catch(()=>({error:{message:'Create failed'}}));setReportFeedback(data.error?.message||'Create failed',true);}finally{setCreateReportBusyState(false);}"
        "});"
...
def _exports_index_html(exports: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "try{const resp=await fetch('/v1/exports',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});if(resp.ok){const data=await resp.json().catch(()=>({export:null}));if(data.export){insertCreatedExport(data.export);document.getElementById('export-id').value='';setExportFeedback(`Created export ${data.export.export_id}.`,false);return;}setExportFeedback('Created export.',false);return;}const data=await resp.json().catch(()=>({error:{message:'Create failed'}}));setExportFeedback(data.error?.message||'Create failed',true);}finally{setCreateExportBusyState(false);}"
        "});"
