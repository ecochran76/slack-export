...
def _attach_semantic_readiness_to_tenants(service, tenants: list[dict[str, Any]]) -> list[dict[str, Any]]:
    try:
    ...
    for tenant in tenants:
        ...
        if name and item.get("db_synced"):
            try:
                payload = service.semantic_readiness(conn, workspace=name)
                workspace_payload = (payload.get("workspaces") or [{}])[0]
                item["semantic_readiness"] = {
                    "status": workspace_payload.get("status"),
                    "tone": workspace_payload.get("tone"),
                    "summary": workspace_payload.get("summary"),
                    "profiles": workspace_payload.get("profiles", []),
                }
            ...
            item["semantic_readiness"] = {
                "status": "not_synced",
                "tone": "neutral",
                "summary": "Semantic readiness is available after the tenant is synced into the DB.",
                "profiles": [],
            }
...
def _search_page_html(
    *,
    ...
    return (
        ...
        ".results{display:grid;gap:12px;margin-top:14px}.result-card{padding:16px;border:1px solid #e4ddd1;border-radius:18px;background:#fcfaf6}.result-card.selected{border-color:#8ab4ff;background:#f7fbff}.result-top{display:flex;flex-wrap:wrap;justify-content:space-between;gap:10px;margin-bottom:10px}.result-side{display:flex;flex-direction:column;align-items:flex-end;gap:8px}"
        ...
        ".selection-panel{display:grid;gap:12px;margin-top:14px;padding:14px;border:1px solid #d8cabb;border-radius:18px;background:#fff9f0}.selection-controls{display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end}.selection-field{min-width:140px;flex:1}.selection-field.compact{max-width:120px}.checkbox-field{display:inline-flex;align-items:center;gap:8px;margin:0 0 5px;font-size:13px;font-weight:700}.checkbox-field input,.select-result input{width:16px;height:16px}.selection-list{display:flex;flex-wrap:wrap;gap:8px}.selection-pill{display:inline-flex;align-items:center;gap:8px;padding:7px 10px;border:1px solid #d4c7b6;border-radius:999px;background:#fffdf9;font-size:12px}.selection-pill button{border:0;background:transparent;color:var(--bad);font-weight:800;cursor:pointer}.select-result{display:inline-flex;align-items:center;gap:8px;margin:0;padding:7px 10px;border:1px solid #c8d8f8;border-radius:999px;background:#edf4ff;color:var(--accent);font-size:12px;font-weight:800;cursor:pointer}"
        ...
        "<div class='selection-panel' id='selected-results-tray'>"
        "<div class='section-title'><h2>Selected results</h2><div class='meta' id='selected-results-count'>0 selected</div></div>"
        "<div class='selection-controls'>"
        "<div class='selection-field'><label for='selected-report-title'>Report title</label><input id='selected-report-title' value='Selected search results' /></div>"
        "<div class='selection-field compact'><label for='selected-before'>Before</label><select id='selected-before'><option value='0'>0</option><option value='1'>1</option><option value='2' selected>2</option><option value='5'>5</option></select></div>"
        "<div class='selection-field compact'><label for='selected-after'>After</label><select id='selected-after'><option value='0'>0</option><option value='1'>1</option><option value='2' selected>2</option><option value='5'>5</option></select></div>"
        "<label class='checkbox-field'><input id='selected-include-text' type='checkbox' checked /> Include text</label>"
        "<button class='btn secondary' id='select-visible-results-button' type='button' disabled>Select visible</button>"
        "<button class='btn secondary' id='deselect-visible-results-button' type='button' disabled>Deselect visible</button>"
        "<button class='btn' id='create-selected-report-button' type='button' disabled>Create report</button>"
        "<button class='btn secondary' id='clear-selected-results-button' type='button' disabled>Clear</button>"
        "</div>"
        "<div class='selection-list' id='selected-results-list'><span class='meta'>Select results from the current page.</span></div>"
        "<div class='hint bad' id='selected-results-error' hidden></div>"
        "<div class='hint' id='selected-results-feedback'></div>"
        "</div>"
        ...
        "const selectedResultsCount=document.getElementById('selected-results-count');"
        "const selectedResultsList=document.getElementById('selected-results-list');"
        "const selectedReportTitle=document.getElementById('selected-report-title');"
        "const selectedBefore=document.getElementById('selected-before');"
        "const selectedAfter=document.getElementById('selected-after');"
        "const selectedIncludeText=document.getElementById('selected-include-text');"
        "const selectVisibleResultsButton=document.getElementById('select-visible-results-button');"
        "const deselectVisibleResultsButton=document.getElementById('deselect-visible-results-button');"
        "const createSelectedReportButton=document.getElementById('create-selected-report-button');"
        "const clearSelectedResultsButton=document.getElementById('clear-selected-results-button');"
        "const selectedResultsError=document.getElementById('selected-results-error');"
        "const selectedResultsFeedback=document.getElementById('selected-results-feedback');"
        "const selectedSearchTargets=new Map();"
        "let selectedReportBusy=false;"
        f"{_BROWSER_ESCAPE_HTML_JS}"
        f"{_BROWSER_BUSY_LABEL_JS}"
        f"{_BROWSER_FETCH_JSON_HELPERS_JS}"
        ...
        "function setSelectedResultsError(message){if(message){selectedResultsError.textContent=message;selectedResultsError.hidden=false;return;}selectedResultsError.textContent='';selectedResultsError.hidden=true;}"
        "function setSelectedResultsFeedback(message,isError=false){selectedResultsFeedback.classList.toggle('bad',Boolean(isError));selectedResultsFeedback.textContent=message||'';}"
        "function actionTargetKey(target){if(!target)return'';return String(target.id||`${target.kind||'result'}:${target.workspace||''}:${target.channel_id||''}:${target.ts||target.source_kind||''}:${target.source_id||target.chunk_id||''}`);}"
        "function actionTargetLabel(target){if(!target)return'Unknown result';if(target.kind==='message')return `${target.workspace||'workspace'} · ${target.channel_id||'channel'} · ${target.ts||'message'}`;if(target.kind==='derived_text')return `${target.workspace||'workspace'} · ${target.source_kind||'source'} · ${target.source_id||'derived text'}`;return target.id||target.kind||'selected result';}"
        "function visibleActionTargets(){const targets=[];for(const checkbox of searchResultsList.querySelectorAll('[data-action-target]')){try{const target=JSON.parse(checkbox.dataset.actionTarget||'{}');const key=actionTargetKey(target);if(key)targets.push({key,target,checkbox});}catch(error){}}return targets;}"
        "function renderSelectedResultsTray(){const count=selectedSearchTargets.size;const visibleTargets=visibleActionTargets();const visibleSelected=visibleTargets.filter((entry)=>selectedSearchTargets.has(entry.key)).length;selectedResultsCount.textContent=`${count} selected`;createSelectedReportButton.disabled=selectedReportBusy||count===0;clearSelectedResultsButton.disabled=selectedReportBusy||count===0;selectVisibleResultsButton.disabled=selectedReportBusy||visibleTargets.length===0||visibleSelected===visibleTargets.length;deselectVisibleResultsButton.disabled=selectedReportBusy||visibleSelected===0;selectedReportTitle.disabled=selectedReportBusy;selectedBefore.disabled=selectedReportBusy;selectedAfter.disabled=selectedReportBusy;selectedIncludeText.disabled=selectedReportBusy;if(!count){selectedResultsList.innerHTML='<span class=\"meta\">Select results from the current page.</span>';return;}selectedResultsList.innerHTML=Array.from(selectedSearchTargets.entries()).map(([key,entry])=>`<span class=\"selection-pill\"><span>${escapeHtml(entry.label)}</span><button type=\"button\" data-remove-selected-target=\"${escapeHtml(key)}\" aria-label=\"Remove ${escapeHtml(entry.label)}\">×</button></span>`).join('');for(const button of selectedResultsList.querySelectorAll('[data-remove-selected-target]')){button.addEventListener('click',()=>{selectedSearchTargets.delete(button.dataset.removeSelectedTarget||'');setSelectedResultsError('');setSelectedResultsFeedback('');renderSelectedResultsTray();syncResultSelectionButtons();});}}"
        "function setSelectedReportBusyState(isBusy){selectedReportBusy=isBusy;setButtonBusyLabel(createSelectedReportButton,isBusy,'creating…');renderSelectedResultsTray();}"
        "function syncResultSelectionButtons(){for(const checkbox of searchResultsList.querySelectorAll('[data-select-target-key]')){const key=checkbox.dataset.selectTargetKey||'';checkbox.checked=selectedSearchTargets.has(key);const card=checkbox.closest('.result-card');if(card)card.classList.toggle('selected',checkbox.checked);}}"
        "function toggleSelectedTarget(target,checked){const key=actionTargetKey(target);if(!key)return;if(checked){selectedSearchTargets.set(key,{target,label:actionTargetLabel(target)});}else{selectedSearchTargets.delete(key);}setSelectedResultsError('');setSelectedResultsFeedback('');renderSelectedResultsTray();syncResultSelectionButtons();}"
        "async function createSelectedReport(){setSelectedResultsError('');setSelectedResultsFeedback('');const selected=Array.from(selectedSearchTargets.values()).map((entry)=>entry.target);if(!selected.length){setSelectedResultsError('Select at least one result first.');renderSelectedResultsTray();return;}const title=(selectedReportTitle.value||'').trim()||`Selected search results ${new Date().toISOString().slice(0,19)}`;const payload={kind:'selected-results',targets:selected,before:Number(selectedBefore.value||2),after:Number(selectedAfter.value||2),include_text:Boolean(selectedIncludeText.checked),max_text_chars:selectedIncludeText.checked?4000:0,title};setSelectedReportBusyState(true);try{const {resp,data}=await fetchJsonResponse('/v1/exports',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});if(!resp.ok){const message=responseErrorMessage(data,'Report creation failed');setSelectedResultsError(message);setSelectedResultsFeedback(message,true);return;}const created=data?.export||{};const exportId=created.export_id||'';const href=created.bundle_url||created.url||(exportId?`/exports/${encodeURIComponent(exportId)}`:'/exports');selectedResultsFeedback.innerHTML=`Created selected-result report${exportId?` <code>${escapeHtml(exportId)}</code>`:''}. <a href=\"${escapeHtml(href)}\">Open report</a>`;}finally{setSelectedReportBusyState(false);}}"
        ...
        "function renderResults(results,state){if(!results.length){renderEmpty(`No results for “${state.query}”.`);renderSelectedResultsTray();syncResultSelectionButtons();return;}searchResultsMeta.textContent=`${results.length} result(s) · ${state.scope==='all'?'all workspaces':state.workspace||'selected workspace'} · ${state.mode}`;searchResultsList.innerHTML=results.map((item)=>{const workspace=item.workspace?`<code>${escapeHtml(item.workspace)}</code>`:'';const source=item._source?`<span class='badge badge-ok'>${escapeHtml(item._source)}</span>`:'';const kind=`<span class='badge badge-neutral'>${escapeHtml(item.result_kind||'result')}</span>`;const score=resultScoreLabel(item);const scoreHtml=score?`<span class='badge badge-warn'>${escapeHtml(score)}</span>`:'';const sourceKind=item.source_kind?`<code>${escapeHtml(item.source_kind)}</code>`:'';const derivedKind=item.derivation_kind?`<code>${escapeHtml(item.derivation_kind)}</code>`:'';const timestamp=item.ts||item.updated_at||item.sort_ts||'';const snippet=escapeHtml(resultSnippet(item)||'');const details=item.result_kind==='message'?[detailItem('channel',item.channel_name||item.channel_id||''),detailItem('timestamp',item.ts||''),detailItem('user',item.user_id||''),detailItem('thread',item.thread_ts||'')].join(''):[detailItem('source id',item.source_id||''),detailItem('updated',item.updated_at||''),detailItem('extractor',item.extractor||''),detailItem('local path',item.local_path||'')].join('');const actions=resultActions(item,state);const target=item.action_target||null;const targetKey=actionTargetKey(target);const selected=targetKey&&selectedSearchTargets.has(targetKey);const selectHtml=targetKey?`<label class=\"select-result\"><input type=\"checkbox\" data-select-target-key=\"${escapeHtml(targetKey)}\" data-action-target=\"${escapeHtml(JSON.stringify(target))}\" ${selected?'checked':''}/> Select</label>`:`<span class=\"badge badge-neutral\">not selectable</span>`;return `<article class=\"result-card ${selected?'selected':''}\"><div class=\"result-top\"><div><div class=\"result-title\">${escapeHtml(resultTitle(item))}</div><div class=\"result-meta\">${kind}${source}${scoreHtml}${workspace}${sourceKind}${derivedKind}</div></div><div class=\"result-side\"><div class=\"meta\">${escapeHtml(timestamp)}</div>${selectHtml}</div></div><p class=\"snippet\">${snippet||'No snippet available.'}</p>${details?`<div class=\"detail-grid\">${details}</div>`:''}${actions?`<div class=\"action-row\">${actions}</div>`:''}</article>`;}).join('');for(const checkbox of searchResultsList.querySelectorAll('[data-action-target]')){checkbox.addEventListener('change',()=>{try{toggleSelectedTarget(JSON.parse(checkbox.dataset.actionTarget||'{}'),checkbox.checked);}catch(error){setSelectedResultsError('Selected result metadata was not valid JSON.');}});}renderSelectedResultsTray();syncResultSelectionButtons();}"
        ...
        "selectVisibleResultsButton.addEventListener('click',()=>{for(const entry of visibleActionTargets()){selectedSearchTargets.set(entry.key,{target:entry.target,label:actionTargetLabel(entry.target)});}setSelectedResultsError('');setSelectedResultsFeedback('');renderSelectedResultsTray();syncResultSelectionButtons();});"
        "deselectVisibleResultsButton.addEventListener('click',()=>{for(const entry of visibleActionTargets()){selectedSearchTargets.delete(entry.key);}setSelectedResultsError('');setSelectedResultsFeedback('');renderSelectedResultsTray();syncResultSelectionButtons();});"
        "createSelectedReportButton.addEventListener('click',()=>{createSelectedReport().catch((error)=>{setSelectedReportBusyState(false);setSelectedResultsError(String(error));setSelectedResultsFeedback(String(error),true);});});"
        "clearSelectedResultsButton.addEventListener('click',()=>{selectedSearchTargets.clear();setSelectedResultsError('');setSelectedResultsFeedback('');renderSelectedResultsTray();syncResultSelectionButtons();});"
        "scopeSelect.value=initialSearchState.scope==='all'?'all':'workspace';workspaceSelect.value=initialSearchState.workspace||workspaceSelect.value;modeSelect.value=initialSearchState.mode||'hybrid';limitSelect.value=String(initialSearchState.limit||20);kindSelect.value=initialSearchState.kind||'';sourceKindSelect.value=initialSearchState.source_kind||'';renderSelectedResultsTray();updateWorkspaceState();updateUrlAndApiLink(currentState());updatePager(currentState(),0,0);if(initialSearchState.query){runSearch(currentState());}"
        "</script></div></body></html>"
    )
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            ...
            if path == "/settings/tenants":
                ...
                try:
                    tenants = _attach_semantic_readiness_to_tenants(service, tenant_status(config_path=config_path))
                except Exception as exc:  # noqa: BLE001
            ...
            if m:
                ...
                try:
                    payload = service.corpus_search_page(
                        ...
                        retrieval_profile_name=query.get("retrieval_profile", [None])[0],
                        ...
                        fusion_method=str(query.get("fusion", ["weighted"])[0]),
            ...
            if path == "/v1/search/profiles":
                try:
                    profiles = service.retrieval_profiles()
                except Exception as exc:  # noqa: BLE001
                    _service_error_response(self, exc, path=path, operation="search.profiles")
                    return
                _json_response(self, 200, {"ok": True, "profiles": profiles})
                return

            def _profile_names_from_query() -> list[str] | None:
                raw = str(query.get("profiles", [""])[0] or "").strip()
                if not raw:
                    return None
                return [value.strip() for value in raw.split(",") if value.strip()]

            if path == "/v1/search/semantic-readiness":
                conn = service.connect()
                try:
                    payload = service.semantic_readiness(
                        conn,
                        profile_names=_profile_names_from_query(),
                        include_commands=query.get("include_commands", ["0"])[0] in {"1", "true", "yes"},
                        command_limit=int(query.get("command_limit", [500])[0]),
                    )
                except Exception as exc:  # noqa: BLE001
                    _service_error_response(self, exc, path=path, operation="search.semantic_readiness")
                    return
            ...
            m = re.fullmatch(r"/v1/workspaces/([^/]+)/search/semantic-readiness", path)
            if m:
                conn = service.connect()
                try:
                    payload = service.semantic_readiness(
                    ...
                    _service_error_response(self, exc, path=path, workspace=m.group(1), operation="search.semantic_readiness")
                    return
                _json_response(self, 200, {"ok": True, "readiness": payload})
                return
        ...
        def do_POST(self):  # noqa: N802
            ...
            if path == "/v1/exports":
                ...
                if kind not in {"channel-day", "selected-results"}:
                    _error_response(self, 400, "BAD_REQUEST", f"Unsupported export kind: {kind}")
                    return
                try:
                    if kind == "selected-results":
                        conn = service.connect()
