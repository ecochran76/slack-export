...
Repo-bundled agent skills for Slack Mirror search, send, channel management, ingest, live-ops, and export workflows live under `agent-skills/` and can be installed with `scripts/install_agent_skills.sh`.
...
- source-label hits now count as stronger ranking evidence for non-generic channel terms, while generic topic labels such as `research` remain body-text evidence rather than source-prior boosts
...
- `/v1/events/follow` now exposes bounded long-poll JSON over the append-only child event journal so Receipts can maintain parent-owned subscriptions and bookmarks without scraping logs or polling derived current-state rows
- the append-only child event journal now includes operational event rows for outbound write results, tenant live-sync actions, and tenant initial-sync/backfill requests so Receipts subscriptions can surface child-owned operational state changes
